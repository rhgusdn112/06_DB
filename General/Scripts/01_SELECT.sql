...
/* SYSDATE/ CRUUENT_DATE , SYSTIMESTAMP / CURRENT)TIMESTAMP */
 ...
 *						 				ms 단위 + 지역 포함
 * - CURRENTTIMESTAMP : 현재 세션(사용자 기반) 시간
 *						 				ms 단위 + 지역 포함
 *
 */
...
-- TO_DATE('날짜문자열', '패턴');
-- 	-> '날짜문자열' 을 '패턴' 형식으로 해석하여 DATE 타입으로 반환
...
/* - 별칭 지정 방법
 * 1) 컬럼명 AS  별칭		: 문자O, 띄어쓰기 안됨, 특수문자 X
 * 
 ...
 * 	 중복을 제거하고 한번만 표시 할 때 사용하는 구문(연산자)
 * */
 ...
 * 1 >> 특정 테이블에서
 * 2 >> 조건식을 만족하는 행을 추려놓고
 * 3 >> 추려진 결과(RESULT SET) 행에서 원하는 컬럼만 조회   
 * */
...
-- EMPLOYEE 테이블에서
-- 급여가 300만 이상 500만 미만의 사원의
-- 사번, 이름, 급여 조회
	
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
WHERE 
	SALARY >= 3000000 AND SALARY <5000000; -- 16행 조회

-- EMPLOYEE 테이블에서
-- 급여가 300만 미만 500만 이상의 사원의
-- 사번, 이름, 급여 조회
...
WHERE HIRE_DATE 
  ...
  AND 
  TO_DATE('2019-12-31', 'YYYY-MM-DD'); -- 결과는 또옥같다
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE 
FROM EMPLOYEE
WHERE DEPT_CODE IN ('D5','D6','D9'); -- 12행 조회
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE 
FROM EMPLOYEE
WHERE DEPT_CODE NOT IN ('D5','D6','D9'); -- 9행 조회(NULL 미포함)
...
-- EMPLOYEE 테이블에서 이름이 '하'로 끝나는 사원의
-- 사번, 이름 조회
 ...
 *		원하는 순서로 정렬 할 때 사용하는 구문 
 * 		
 * [작성법]
 * 3) SELECT 		컬럼명  					 (특정 컬럼만 선택해서 볼거야)
 * 1) FROM 	 		테이블명						 (어떤 테이블에서)
 * 2) WHERE  		조건식  					 (조건을 만족하는 행만 선택)
 * 4)	ORDER BY									 (조회된 결과를 정렬 할거야)
 * 		컬럼명 | 별칭 | 컬럼순서(숫자)
 * 		[ASC/DESC] 								 (오름차순/내림차순)
 * 		[NULLS FIRST / NULLS LAST] (NULL 위치 지정)
 * 
 * ***** ORDER BY 절은 무조건 SELECT 마지막에 수행 된다 !!! *****
 * 
 * */
...
-- 직급 코드가 'J4', 'J5', 'J6' 인 사원 을
-- 사번, 이름, 직급 코드 조회하기
-- 단, 이름 오름차순으로 조회

SELECT EMP_ID "사번", EMP_NAME "이름", JOB_CODE "직급 코드" 
FROM EMPLOYEE
...
--> SELECT절에 작성되지 않아도
-- 이전에 FROM절이 해석 되어있기 때문에 정렬이 가능하다
