...
--> DEPT_CODER, DEPT_ID가 일치하는 행끼리 한 행으로 합쳐짐
...
FROM DEPT_TITLE 부서명, LOCAL_NAME 지역명
FROM DEPARTMENT, LOCATION
WHERE LOCATION_ID = LOCAL_CODE;

-- 부서명이 '해외영업2부'인 사원의
-- 사번(EMP_ID), 이름(EMP_NAME), 부서명(DEPT_TITLE)
-- 사번 오름차순으로 조회
...
--> JOB_CODE 컬럼명이 같아서 어떤 테이블의 컬럼인지 구분 할 수 없기 때문
...
SELECT
			EMP_ID,
			EMP_NAME,
			E.JOB_CODE, -- 어떤 테이블의 컬럼을 조회할 지 별칭 작성!
			JOB_NAME
...
--> 직접 사용보단 NATURAL JOIN의 실패 결과로 보여지는 모습이다!
--	라고 알아두는게 좋음
...
WHERE EMP.MANAGER_ID = MGR.EMP_ID(+); -- LEFT JOIN
...
-- 2) DEPARTMENT(LOCAION_ID), LOCATION(LOCAL_CODE) JOIN
...
-- 9. 보너스포인트가 없는 직원들 중에서 직급코드가 J4와 J7인 
