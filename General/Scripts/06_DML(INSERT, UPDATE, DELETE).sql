...
SELECT COUNT(*) -- 전체 행의 개수가 23행 카운트
FROM EMPLOYEE2;
...
ROLLBACK;
--> COMMIT 되지 않은 DML(INSRT,UPDATE,DELETE) 취소하기
	-- == DML 수행 전으로 되돌리기
---------------------------------------
...
UPDATE EMPLOYEE2
SET 
		(SALARY,BONUS) = ( -- 다중열 서브쿼리
				SELECT SALARY, BONUS
				FROM EMPLOYEE2
				WHERE EMP_NAME = '유재식'
		)
WHERE 
		EMP_NAME IN ('정중하', '노옹철', '전형돈', '하동운');
-- 4행 수정됨
...
SELECT EMP_NAME, SALARY, BONUS
FROM EMPLOYEE2
WHERE EMP_NAME IN ('정중하','노옹철','전형돈','하동운');
...
USING EMP_M02 ON(EMP_M01.EMP_ID = EMP_M02.EMP_ID) 
									-- 중복 확인용 KEY
WHEN MATCHED THEN -- 중복 확인용 KEY가 일치하면(매치되면)
UPDATE SET				-- 수정하겠다
