...
/* 자료형
    NUMBER : 숫자형(정수, 실수) 크기는 21자리(10해)까지가능
    CHAR(크기) : 고정길이 문자형 (2000BYTE) (주민번호)
        -> ex) CHAR(10) 컬럼에 'ABC' 3BYTE 문자열만 저장해도 10BYTE 저장공간을 모두 사용. 
        
    VARCHAR2(크기) : 가변길이 문자형 (4000 BYTE) (일을 두번 해서 시간이 오래걸림)                
        -> ex) VARCHAR2(10) 컬럼에 'ABC' 3BYTE 문자열만 저장하면 나머지 7BYTE를 반환함. 
        
    ** CHAR / VARCHAR2 주의사항 **
    -  오라클은 기본적으로 UTF-8 문자 인코딩을 사용하는데
        UTF-8은 영어/숫자 1BYTE, 한글/한자 3BYTE로 저장하기 때문에
        한글 데이터 저장시 크기 계산에 주의해야 한다!!!
        
    NVARCHAR2(문자 개수) : "가변"길이 문자형 (4000BYTE) 
          -> BYTE 크기 단위가 아닌 글자 수(문자 개수)로 크기를 지정하는 타입
              
          -> ex) VARCHAR2(3)  : 가변길이 문자형 크기 3BYTE 
                    NVARCHAR2(3) : 가변길이 문자형 문자 3개까지 저장 가능(한글,영어 상관없이)
...
-- 여러 SQL 한번에 실행 : 블럭(드래그)으로 선택 후  ALT + X
...
-- 가입일 -> SYSDATE를 활용
INSERT INTO "MEMBER"
VALUES('MEM02', 'QWER1234', '김영희', 
         '980808-2345678', CURRENT_DATE);
...
CREATE TABLE USER_USED_UK(
   ...
   * - 제약조건종류(컬럼) : 어떤 컬럼에 제약조건을 설정할 지 작성 
   */
...
-- ORA-00001: 무결성 제약 조건(KH_JWK.USER_ID_U)에 위배됩니다
...
INSERT INTO USER_USED_UK
VALUES(1, NULL, 'pass01', '나길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
--  ORA-00001: 무결성 제약 조건(KH_JWK.USER_NAME_U)에 위배됩니다
...
-- ORA-00001: 무결성 제약 조건(KH_JWK.USER_ID_NAME_U)에 위배됩니다
...
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(KH_JWK.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("KH_JWK"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-00001: 무결성 제약 조건(KH_JWK.PK_USERNO_USERID)에 위배됩니다
...
-- ORA-01400: NULL을 ("KH_JWK"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
CREATE TABLE USER_USED_FK(
  ...
  GRADE_CODE NUMBER -- 해당 컬럼에 작성될 수 있는 값은
                             -- USER_GRADE 테이블의 
                             -- PK컬럼값(GRADE_CODE)만 작성하게 제한하고 싶다!
                              --> FK 제약조건 설정
...
-- ORA-02291: 무결성 제약조건(KH_JWK.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
-- 부모 키가 없습니다
...
FROM USER_GRADE
WHERE GRADE_CODE = '10';
-- ORA-02292: 무결성 제약조건(KH_JWK.GRADE_CODE_FK1)이 위배되었습니다
-- - 자식 레코드가 발견되었습니다
...
-- ORA-02290: 체크 제약조건(KH_JWK.GENDER_CHECK)이 위배되었습니다
