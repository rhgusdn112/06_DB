...
ALTER TABLE DEPT_COPY
ADD (LNAME VARCAHR2(30) DEFAULT '한국');
...
SELECT DEPT_TITLE, 
			 LENGTH(DEPT_TITLE) "문자열 길이" ,
			 LENGTHB(DEPT_TITLE) "문자열의 바이트 크기"
FROM DEPT_COPY;
...
SELECT * FROM DEPT_COPY; -- 'D0' 삭제되지 않음!
...
DROP TABLE TB1;
-- ORA-02449: 외래 키에 의해 참조되는 고유/기본 키가 
-- 테이블에 있습니다
-- == PK 컬럼이 다른 테이블에 참조를 당하고 있다

-- 해결 방법 1) 참조하는 자식 테이블(TB2)을 삭제하거나 FK 제약조건을 삭제한다


-- 해결 방법 2) CASCADE CONSTRAINTS 옵션 사용하기
--> 테이블 삭제 시, 연결되어 있는 FK 제약 조건을 같이 삭제시킴
...
ALTER TABLE DEPT_COPY RENAME COLUMN 
DEPT_TITLE TO DEPT_NAME;

SELECT * FROM DEPT_COPY;

-- 2) 제약조건명 변경 : ALTER TABLE 테이블명 RENAME CONSTRAINT 제약조건명 TO 변경명;
-- DEPT_COPY_PK -> PK_DCOPY로 변경
